...
class ViewController: UIViewController {
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        moonCenterPoint.position = earth.position
        let moon = planet(geometry: SCNSphere(radius: 0.02), diffuse: UIImage(named: "moon")!, specular: nil, emission: nil, normal: nil, position: SCNVector3(0.35,0,0))
        ...
        let jupiterRotation = planetRotation(duration: 7)
        let saturnRotation = planetRotation(duration: 6)
        let uranusRotation = planetRotation(duration: 5)
        let neptuneRotation = planetRotation(duration: 4)
    ...
    func planetRotation(duration:TimeInterval)-> SCNAction{
        return SCNAction.repeatForever(SCNAction.rotateBy(x: 0, y: CGFloat(360.degreesToRadians), z: 0, duration: duration))
    }
